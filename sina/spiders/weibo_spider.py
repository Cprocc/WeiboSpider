...
from sina.items import TweetsItem, FansInformationItem, RelationshipsItem, CommentItem, CommentsInformationItem, \
...
class WeiboSpider(Spider):
    ...
    #     """ 抓取个人信息 """
    #     information_item = FansInformationItem()
    ...
    #     # 交给抓取粉丝的任务
    #     for uid in uids:
    #         yield Request(url="https://weibo.cn/%s/info" % uid, callback=self.parse_fans_information)
    #         '''
    #         relationships_item = RelationshipsItem()
    #         relationships_item['crawl_time'] = int(time.time())
    ...
    def parse_comment(self, response):
        ...
        for comment_node in comment_nodes:
            ...
            # 给comment_info 用来抓评论信息
            yield Request(url="https://weibo.cn/%s/info" % uid, callback=self.parse_comments_information)
            content = comment_node.xpath('.//span[@class="ctt"]').xpath('string(.)').extract_first()
            ...
            yield comment_item
    #
    # def parse_fans_information(self, response):
    #     """ 抓取个人信息 """
    #     fans_information_item = FansInformationItem()
    #     fans_information_item['crawl_time'] = int(time.time())
    #     selector = Selector(response)
    #     fans_information_item['_id'] = re.findall('(\d+)/info', response.url)[0]
    #     text1 = ";".join(selector.xpath('body/div[@class="c"]//text()').extract())  # 获取标签里的所有text()
    #     nick_name = re.findall('昵称;?[：:]?(.*?);', text1)
    #     gender = re.findall('性别;?[：:]?(.*?);', text1)
    #     place = re.findall('地区;?[：:]?(.*?);', text1)
    ...
    #     labels = re.findall('标签;?[：:]?(.*?)更多>>', text1)
    #     if nick_name and nick_name[0]:
    #         fans_information_item["nick_name"] = nick_name[0].replace(u"\xa0", "")
    #     if gender and gender[0]:
    #         fans_information_item["gender"] = gender[0].replace(u"\xa0", "")
    #     if place and place[0]:
    #         place = place[0].replace(u"\xa0", "").split(" ")
    #         fans_information_item["province"] = place[0]
    #         if len(place) > 1:
    #             fans_information_item["city"] = place[1]
    #     if briefIntroduction and briefIntroduction[0]:
    #         fans_information_item["brief_introduction"] = briefIntroduction[0].replace(u"\xa0", "")
    #     if birthday and birthday[0]:
    #         fans_information_item['birthday'] = birthday[0]
    #     if sex_orientation and sex_orientation[0]:
    #         if sex_orientation[0].replace(u"\xa0", "") == gender[0]:
    #             fans_information_item["sex_orientation"] = "同性恋"
    #         else:
    #             fans_information_item["sex_orientation"] = "异性恋"
    #     if sentiment and sentiment[0]:
    #         fans_information_item["sentiment"] = sentiment[0].replace(u"\xa0", "")
    #     if vip_level and vip_level[0]:
    #         fans_information_item["vip_level"] = vip_level[0].replace(u"\xa0", "")
    #     if authentication and authentication[0]:
    #         fans_information_item["authentication"] = authentication[0].replace(u"\xa0", "")
    #     if labels and labels[0]:
    #         fans_information_item["labels"] = labels[0].replace(u"\xa0", ",").replace(';', '').strip(',')
    #     request_meta = response.meta
    #     request_meta['item'] = fans_information_item
    #     yield Request(self.base_url + '/u/{}'.format(fans_information_item['_id']),
    #                   callback=self.parse_further_fans_information,
    #                   meta=request_meta, dont_filter=True, priority=1)
    #
    # def parse_further_fans_information(self, response):
    #     text = response.text
    #     fans_information_item = response.meta['item']
    #     tweets_num = re.findall('微博\[(\d+)\]', text)
    #     if tweets_num:
    #         fans_information_item['tweets_num'] = int(tweets_num[0])
    #     follows_num = re.findall('关注\[(\d+)\]', text)
    #     if follows_num:
    #         fans_information_item['follows_num'] = int(follows_num[0])
    #     fans_num = re.findall('粉丝\[(\d+)\]', text)
    #     if fans_num:
    #         fans_information_item['fans_num'] = int(fans_num[0])
    #     yield fans_information_item

    # 获取评论信息
    def parse_comments_information(self, response):
        """ 抓取个人信息 """
        comments_information_item = CommentsInformationItem()
        comments_information_item['crawl_time'] = int(time.time())
        selector = Selector(response)
        comments_information_item['_id'] = re.findall('(\d+)/info', response.url)[0]
        text1 = ";".join(selector.xpath('body/div[@class="c"]//text()').extract())  # 获取标签里的所有text()
        nick_name = re.findall('昵称;?[：:]?(.*?);', text1)
        gender = re.findall('性别;?[：:]?(.*?);', text1)
        place = re.findall('地区;?[：:]?(.*?);', text1)
        ...
        labels = re.findall('标签;?[：:]?(.*?)更多>>', text1)
        if nick_name and nick_name[0]:
            comments_information_item["nick_name"] = nick_name[0].replace(u"\xa0", "")
        if gender and gender[0]:
            comments_information_item["gender"] = gender[0].replace(u"\xa0", "")
        if place and place[0]:
            place = place[0].replace(u"\xa0", "").split(" ")
            comments_information_item["province"] = place[0]
            if len(place) > 1:
                comments_information_item["city"] = place[1]
        if briefIntroduction and briefIntroduction[0]:
            comments_information_item["brief_introduction"] = briefIntroduction[0].replace(u"\xa0", "")
        if birthday and birthday[0]:
            comments_information_item['birthday'] = birthday[0]
        if sex_orientation and sex_orientation[0]:
            if sex_orientation[0].replace(u"\xa0", "") == gender[0]:
                comments_information_item["sex_orientation"] = "同性恋"
            else:
                comments_information_item["sex_orientation"] = "异性恋"
        if sentiment and sentiment[0]:
            comments_information_item["sentiment"] = sentiment[0].replace(u"\xa0", "")
        if vip_level and vip_level[0]:
            comments_information_item["vip_level"] = vip_level[0].replace(u"\xa0", "")
        if authentication and authentication[0]:
            comments_information_item["authentication"] = authentication[0].replace(u"\xa0", "")
        if labels and labels[0]:
            comments_information_item["labels"] = labels[0].replace(u"\xa0", ",").replace(';', '').strip(',')
        request_meta = response.meta
        request_meta['item'] = comments_information_item
        yield Request(self.base_url + '/u/{}'.format(comments_information_item['_id']),
                      callback=self.parse_further_comments_information,
                      meta=request_meta, dont_filter=True, priority=1)

    def parse_further_comments_information(self, response):
        text = response.text
        comments_information_item = response.meta['item']
        tweets_num = re.findall('微博\[(\d+)\]', text)
        if tweets_num:
            comments_information_item['tweets_num'] = int(tweets_num[0])
        follows_num = re.findall('关注\[(\d+)\]', text)
        if follows_num:
            comments_information_item['follows_num'] = int(follows_num[0])
        fans_num = re.findall('粉丝\[(\d+)\]', text)
        if fans_num:
            comments_information_item['fans_num'] = int(fans_num[0])
        yield comments_information_item
    ...
    def parse_like(self, response):
        ...
        for comment_node in comment_nodes:
            try :
                ...
                yield Request(url="https://weibo.cn/%s/info" % uid, callback=self.parse_like_information)
                created_at = comment_node.xpath('.//span[@class="ct"]/text()').extract_first()
                comment_item['created_at'] = time_fix(created_at.split('\xa0')[0])
                yield comment_item

    def parse_like_information(self, response):
        """ 抓取个人信息 """
        comments_information_item = LikeInformationItem()
        comments_information_item['crawl_time'] = int(time.time())
        selector = Selector(response)
        comments_information_item['_id'] = re.findall('(\d+)/info', response.url)[0]
        text1 = ";".join(selector.xpath('body/div[@class="c"]//text()').extract())  # 获取标签里的所有text()
        ...
        request_meta['item'] = comments_information_item
        yield Request(self.base_url + '/u/{}'.format(comments_information_item['_id']),
                      callback=self.parse_further_like_information,
                      meta=request_meta, dont_filter=True, priority=1)

    def parse_further_like_information(self, response):
        text = response.text
        comments_information_item = response.meta['item']
        tweets_num = re.findall('微博\[(\d+)\]', text)
        if tweets_num:
    ...
    def parse_rep(self, response):
        ...
        if response.url.endswith('page=1'):
            ...
            if all_page:
                ...
                for page_num in range(2, all_page + 1):
                    page_url = response.url.replace('page=1', 'page={}'.format(page_num))
                    yield Request(page_url, self.parse_rep, dont_filter=True, meta=response.meta)
        selector = Selector(response)
        comment_nodes = selector.xpath('//div[@class="c"]')
        for comment_node in comment_nodes:
            try:
                ...
                yield Request(url="https://weibo.cn/%s/info" % uid, callback=self.parse_rep_information)
                ...
                created_at = comment_node.xpath('.//span[@class="ct"]/text()').extract_first()
                comment_item['created_at'] = time_fix(created_at.split('\xa0')[1])
            except:
                pass
            else:
                try :
                    comment_item['created_device'] = created_at.split('\xa0')[2]
                except:
                    pass
                else:
                    pass
    ...
    def parse_rep_information(self, response):
        """ 抓取个人信息 """
        comments_information_item = RepostInformationItem()
        comments_information_item['crawl_time'] = int(time.time())
        selector = Selector(response)
        comments_information_item['_id'] = re.findall('(\d+)/info', response.url)[0]
        text1 = ";".join(selector.xpath('body/div[@class="c"]//text()').extract())  # 获取标签里的所有text()
        ...
        request_meta['item'] = comments_information_item
        yield Request(self.base_url + '/u/{}'.format(comments_information_item['_id']),
                      callback=self.parse_further_rep_information,
                      meta=request_meta, dont_filter=True, priority=1)

    def parse_further_rep_information(self, response):
        text = response.text
        comments_information_item = response.meta['item']
        tweets_num = re.findall('微博\[(\d+)\]', text)
        if tweets_num:
